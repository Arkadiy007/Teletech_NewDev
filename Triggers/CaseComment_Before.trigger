...
trigger CaseComment_Before on CaseComment (before insert, before update) {
    ...
    */
    ...
    cases = [select Id, Stage_Status__c, Status, IsClosed, ClosedDate, RecordTypeId, RecordType.Name from Case where Id in :caseIds];
   ...
   for (CaseComment cc: Trigger.new) {
    ...
    if(System.label.TurnOffStageStatus != 'Yes') {
        ...
        if (!updateCases.isEmpty()) {
            ...
            /*for (CaseComment ccWithError: ccsWithErrors) {
                ccWithError.addError('You can not add a Case Comment after the Case has been closed for 90 days.<br/>Click Cancel to return to the Case.');
            }*/
    ...
    if (system.label.TurnOffValidation != 'Yes') {
        for (CaseComment cc: Trigger.new) {
            ...
            if (Trigger.isInsert && c.RecordTypeId == caClosedCaseRT && c.ClosedDate.Date().daysBetween(System.Today()) > 90) 
                cc.addError('You can not add a Case Comment after the Case has been closed for 90 days.<br/>Click Cancel to return to the Case.');
