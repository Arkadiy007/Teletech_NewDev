...
trigger OCPSuppressions_Before on OCP_Suppression__c (before insert)  { 
	String leaseLoyaltyRecordTypeId = RecordtypeUtil.getObjectRecordTypeId(Case.SObjectType, 'Lease Loyalty');
	...
	String query = 'SELECT ID, VIN__c, Segment_Mailed__c FROM Case WHERE Status != \'Closed\' AND Subject LIKE \'%Lease Loyalty%\' AND RecordTypeId =:leaseLoyaltyRecordTypeId AND VIN__c IN :vins AND Segment_Mailed__c IN :leaseTypes AND LastModifiedDate > ' + ocpSuppressionSettings.Time_Period_for_Cases__c + ' LIMIT 50000';
	//List<Case> leaseLoyaltyOpenCases = [SELECT ID, VIN__c, Segment_Mailed__c FROM Case WHERE Status != 'Closed' AND RecordTypeId =:leaseLoyaltyRecordTypeId AND VIN__c IN :vins AND Segment_Mailed__c IN :leaseTypes];
	List<Case> leaseLoyaltyOpenCases = Database.query(query);
	if (leaseLoyaltyOpenCases.size() == 50000){
		boolean all = false;
		while(!all){
			query = 'SELECT ID, VIN__c, Segment_Mailed__c FROM Case WHERE Status != \'Closed\' AND RecordTypeId =:leaseLoyaltyRecordTypeId AND VIN__c IN :vins AND Segment_Mailed__c IN :leaseTypes AND LastModifiedDate > ' + OcpSuppressionSettings.Time_Period_for_Cases__c + 'AND ID NOT IN :leaseLoyaltyOpenCases LIMIT 50000';
			List<Case> leaseLoyaltyOpenCases_buf = Database.query(query);
			leaseLoyaltyOpenCases.addAll(leaseLoyaltyOpenCases_buf);
			if (leaseLoyaltyOpenCases_buf.size() < 50000){
				all = true;
			}
		}
	}
