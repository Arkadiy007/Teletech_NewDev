...
1.3 -  Anna Koseykina   03/27/2015 Changed some logic for one time load and old cases
***********************************************************************/
trigger Case_Categorization_Before on Case_Categorization__c (before insert, before update) {
    ...
    cases = new Map<Id,Case>([select Id, Stage_Status__c, IsClosed, Reopen_Date_for_Category__c, Categorizations__c, Status from Case where Id in :caseIds]);
    ...
    if (Trigger.isUpdate && system.label.TurnOffValidation != 'Yes') {
        ...
        for (Case_Categorization__c caseCat : Trigger.new) {
            ...
            for (String field : fieldNames) {
				if(mapCases.get(caseCat.Case__c).Reopen_Date_for_Category__c != null){
					if (caseCat.get(field) != old.get(field) && caseCat.CreatedDate < mapCases.get(caseCat.Case__c).Reopen_Date_for_Category__c) {
						caseCat.addError('Case Categorization cannot be edited on reopened Case if it was added before Case was closed.');
					}
				}else if (caseCat.get(field) != old.get(field) && mapCases.get(caseCat.Case__c).Status == 'Closed') {
