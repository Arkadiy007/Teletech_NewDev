/**********************************************************************
  ...
  1.4 - Vlad Martynenko	03/05/2015 Adding new logic for live agent surveys to populate survey response records.
  1.5. - Vlad Martynenko 08/07/2015 Added logic for Epsilon Surveys population
 ***********************************************************************/
trigger Surveys_After on Surveys__c(After insert, After Update) {

	Set<Id> caseids = new Set<Id> ();
	List<Case> epsilonCasesToUpsert = new List<Case> ();
	...
	String liveAgentTypeId = RecordtypeUtil.getObjectRecordTypeId(Surveys__c.SObjectType, 'Live Agent');
	List<Surveys__c> liveAgentSurveys = new List<Surveys__c> ();
	...
	if (Trigger.isInsert) {
		...
		for (Surveys__c s : Trigger.new) {
			...
			if (s.RecordTypeId == liveagentTypeId) {
				liveAgentSurveys.add(s);
			}
		...
		// Populate new SurveyResponse records for each of new live agent survey       
		new SurveysAfterTriggerHelper(liveagentsurveys);
	}
		...
		for (Surveys__c s : Trigger.new) {
			...
			((s.DQR_Dealer_Contacted__c == 'yes' && s.DQR_Dealer_Rating__c != null)
			 || (s.DQR_Dealer_Contacted__c == 'no')) &&
				...
				updatesurvey.Hot_Alert_Created__c = true;
				updatesurvey.CastIron_ETL_Status__c = false;
				if (s.SurveyGizmo_Response_Date__c == null) {
