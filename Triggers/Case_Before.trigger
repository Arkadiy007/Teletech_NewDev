...
2.4 - Anna Koseykina 03/27/2015 Add logic for Date time of reopen
2.5 - Vladimir Martynenko 07/07/2015 Add logic for reopen Case if Case_Dealer_Disposition__c equals to REJECTED or NOTACCEPTED and GUID is not null
***********************************************************************/
trigger Case_Before on Case (before insert, before update) {
   ...
   // public Map<Id, Id> caseRecordTypes = new Map<ID, ID>();
    ...
    Map<Id, Vehicle__c> caseVehicles;
   ...
   // Id techLineRecordTypeId = RecordtypeUtil.getObjectRecordTypeId(Case.SObjectType, 'TECH LINE Cases');
    ...
    List<Account> customerAccounts = [select Id, PersonContactId, Customer_Id__c from Account where customer_id__c in :customerIds];
    ...
    List<Account> dealers = [select Id, Dealer_External_Id__c from Account where RecordType.Name = 'Dealer' and Dealer_External_Id__c in :dealerExternalIds];
    ...
    List<Vehicle__c> vehicleIdList = [select Id, Vehicle_Identification_Number__c from Vehicle__c where Vehicle_Identification_Number__c in :vehicleVINs];
    Map<String,Vehicle__c> vehicleIdMap = new Map<String,Vehicle__c>();
    for (Vehicle__c vehicle: vehicleIdList) {
    ...
    */
     // Set DTS Inspection Date
    for (Case c: Trigger.new) {
      if (c.RecordtypeId == '012F0000000y9y7' || c.RecordtypeId == '012F0000000yCuJ' ||
           c.RecordtypeId == '012F0000000yFmQ') {
	...
	if(treadCodeSettings == null || treadCodeSettings.Calculate_TREAD__c == null || treadCodeSettings.User_ID_not_Fire__c == null){
        ...
        treadCodeSettings.User_ID_not_Fire__c = '005A0000001Y7Ek'; //Manage Service ID
        upsert treadCodeSettings;
        treadCodeSettings = TREAD_Codes_Logic__c.getOrgDefaults();
	}
	Set<String> caCases = new Set<String> ();
    try {
        for(String s : treadCodeSettings.CasesTypes__c.split(',')){
            caCases.add(rtInfosByName.get(s).getRecordTypeId());
        }
	...
	for (Case c : Trigger.New) {
		if(treadCodeSettings.Calculate_TREAD__c == true && !usersNotExecute.contains(UserInfo.getUserId().substring(0, 15))){
			if(caCases.contains(c.RecordTypeId)){
				...
				if(trigger.IsUpdate && trigger.oldMap.get(c.id).Status == 'Closed' && c.Status != 'Closed'){
					c.Reopen_Date_for_Category__c = Datetime.now();
				}
			}
		}
	...
	if(Trigger.isUpdate){
		for(Case cs : Trigger.NEW){
			if(cs.Status == 'Closed' && (cs.Case_Dealer_Disposition__c == 'CACHE' || cs.Case_Dealer_Disposition__c == 'REJECTED') && cs.Case_GUID__c != null){
				cs.Status = 'Open';
			}
		}
	}
