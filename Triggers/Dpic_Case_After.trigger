...
// Map for closed DPIC cases
  if (closedCaseIDs.size() > 0) {
      Map<ID, Case> mapClosedDPICCases = new Map<ID, Case>([select id,  origin, contact.email, channel__c, contact.firstname, contact.lastname, owner.name, contact.name, account.name from case where id in :closedCaseIDs]);
           ...
           if (!dpicInsertList.isEmpty()) {
               ...
               for (integer row = 0; row < dpicInsertList.size(); row++)    {
                    if (dpicInsertList[row].isSuccess())  {
                        ...
                        dpicsListMap.get(row).ContactName__c, dpicsListMap.get(row).AgentName__c); 
