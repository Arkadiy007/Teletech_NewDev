/**********************************************************************
  ...
  1.18 - Arkadiy Sychev   12/12/2014 Added notifications creation for updated Cases
  1.19 - Arkadiy Sychev   12/12/2014 Implemented additional logic on notifications creation for updated Cases
  1.20 - Anna Koseykina   18/12/2014 Added functionality for MajorComponentCode definition when Case is Closed
  1.21 - Anna Koseykina   1/6/2015 Fixed some errors on TREAD logic
  1.22 -Vivek Batham  12/24/2014 Added logic for Chatter post when a DTS notification is confirmed
  1.23 - Anna Koseykina   1/28/2015 Made TREAD Codes logic efficient and working only for CA cases
  1.24 - Anna Koseykina   2/2/2015 Moved all logic into separated helper class CaseAfterHelper
  1.25 - Arkadiy Sychev   03/25/2015 Return notifications creation logic for updated Cases
  1.26 - Anna Koseykina   3/25/2015 Added logic for updating CaseWasClosed field of Case Categorizations when Case is closed
 ***********************************************************************/
...
trigger Case_After on Case(after insert, after update) {
	...
	//Vlad Martynenko, 08.21.2015
	if(Trigger.isInsert){
		DQRCasesHandler.processCases(Trigger.new);
	}
	...
	if (Trigger.new.size() == 1) {
		...
		for (Case c : Trigger.new) {
			if (c.RecordTypeId == techlineCasesId) {
				...
				//Insert a Task according to the Follow Up date field
				if (Trigger.isInsert || c.AssignmentLevel__c != Trigger.oldMap.get(c.ID).AssignmentLevel__c) {
					if (c.Follow_Up_Date__c != null && c.Issue_Type__c == 'FULL' &&
					    c.AssignmentLevel__c != null && c.AssignmentLevel__c.contains('|')) {
						List<String> codes = c.AssignmentLevel__c.split('|');
						if (Integer.valueOf(codes[1]) > 0 && Integer.valueOf(codes[1]) < 9) {
							String taskTechlineId = RecordtypeUtil.getObjectRecordTypeId(Task.SObjectType, 'TECH LINE');


							Task followUpTask = new Task();
							followUpTask.RecordTypeId = taskTechlineId;
							followUpTask.OwnerId = c.OwnerId;
							followUpTask.WhatId = c.Id;
							followUpTask.ActivityDate = c.Follow_Up_Date__c;
							followUpTask.Subject = 'TECH LINE Case Follow Up';
							followUpTask.Status = 'Not Started';
							followUpTask.Priority = 'Normal';
							followUpTask.Description = 'Case: ' + c.CaseNumber;
	...
	// 1.24 - Return notifications creation logic for updated Cases
	CaseAfterHelper caseAfterHelperInstance = new CaseAfterHelper();
	caseAfterHelperInstance.notifications(Trigger.isUpdate, Trigger.new, Trigger.oldMap);
	if (CaseAfterHelper.firstRun || Test.isRunningTest()) {
		caseAfterHelperInstance.tigerTeamSurveys(Trigger.new, Trigger.oldMap, Trigger.isUpdate);
