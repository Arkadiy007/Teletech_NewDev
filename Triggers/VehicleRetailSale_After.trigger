...
trigger VehicleRetailSale_After on Vehicle_Retail_Sale__c (after insert, after update) {
    ...
    Map<Id,Vehicle__c> vehciles = new Map<Id,Vehicle__c>([SELECT Id, Make_Name__c, Model_Year__c, Model_Line_Name__c FROM Vehicle__c WHERE Make_Name__c = :welcomeCallsSetting.Make__c AND Model_Year__c = :welcomeCallsSetting.Model_Year__c AND Id IN :vehicleIds]); //AND Model_Line_Name__c IN :modelLine 
	for (Integer i = 0; i < Trigger.new.size(); i++) {
